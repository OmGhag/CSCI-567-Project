...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    for _ in range(1, n_cluster):
        dists = np.min(np.sum((x[:, np.newaxis, :] - x[centers])**2, axis=2), axis=1)
        dists /= np.sum(dists)
        cumsum = np.cumsum(dists)
        rnd = generator.rand()
        new_center = np.where(cumsum > rnd)[0][0]
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        for iteration in range(self.max_iter):
            ...
            new_centers = np.array([x[labels == k].mean(axis=0) for k in range(self.n_cluster)])
            if np.linalg.norm(new_centers - centers) < self.e:
                break
            centers = new_centers
...
class KMeansClassifier():
    ...
    def fit(self, x, y, centroid_func=get_lloyd_k_means):
        ...
        centroid_labels = np.array([np.argmax(np.bincount(y[labels == k])) for k in range(self.n_cluster)])
    ...
    def predict(self, x):
        ...
        # Compute distances from test points to stored centroids
        dists = np.sum((x[:, np.newaxis, :] - self.centroids[np.newaxis, :, :])**2, axis=2)

        # Find nearest centroid for each point
        nearest_centroids = np.argmin(dists, axis=1)

        # Get the labels of nearest centroids
        pred_labels = self.centroid_labels[nearest_centroids]
...
def transform_image(image, code_vectors):
    ...
    ##############################################################################
    original_shape = image.shape  # Save original shape (H, W, 3)
    pixels = image.reshape(-1, 3)  # Reshape to (H*W, 3)
    
    dists = np.sum((pixels[:, np.newaxis, :] - code_vectors[np.newaxis, :])**2, axis=2)
    nearest_code_vectors = np.argmin(dists, axis=1)
    quantized_pixels = code_vectors[nearest_code_vectors]
    
    return quantized_pixels.reshape(original_shape).astype(np.uint8)
