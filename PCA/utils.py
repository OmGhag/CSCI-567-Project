...
def find_most_sim_word(word, words, E):
    ...
    ###############################################################################################################
    word_emb = find_embedding(word, words, E)
    similarity = np.dot(E, word_emb)
    
    word_index = np.where(word == words)[0][0]
    similarity[word_index] = -np.inf  # Set to -infinity so it won't be selected
...
def find_info_ev(v, words, k=20):
    ...
    ##########################################################################################################################
    v = np.abs(v)
    sorted_indices = np.argsort(-v)  # Sort in decreasing order
    top_k_indices = sorted_indices[:k]
    info = [words[i] for i in top_k_indices]
...
def get_projections(word_seq, words, E, w):
    ...
    ###############################################################################################################
    word_seq_embeddings = [find_embedding(word, words, E) for word in word_seq]
    w_normalized = w / np.linalg.norm(w)
    proj_seq = [np.dot(emb, w_normalized) for emb in word_seq_embeddings]
...
def comparative_projections(word_seq, words, E, wd1, wd2):
    ...
    w = w1 - w2
    w_normalized = w / np.linalg.norm(w)
    proj_seq = get_projections(word_seq, words, E, w_normalized)
...
def find_most_sim_word_w(w, words, E, ids):
    ...
    #########################################################################################################################################
    similarity = np.dot(E, w)
    for id in ids:
        similarity[id] = -np.inf  # Exclude these indices by setting similarity to -infinity
    max_index = np.argmax(similarity)
    most_sim_word = words[max_index]
...
def find_analog(wd1, wd2, wd3, words, E):
    ...
    #########################################################################################################################################
    wd1_emb = find_embedding(wd1, words, E)
    wd2_emb = find_embedding(wd2, words, E)
    wd3_emb = find_embedding(wd3, words, E)
    w = wd2_emb - wd1_emb + wd3_emb
    w_normalized = w / np.linalg.norm(w)
    ids = [np.where(wd1 == words)[0][0], np.where(wd2 == words)[0][0], np.where(wd3 == words)[0][0]]
    wd4 = find_most_sim_word_w(w_normalized, words, E, ids)
...
def similarity(wd1, wd2, words, E):
    ...
    wd1_emb = find_embedding(wd1, words, E)
    wd2_emb = find_embedding(wd2, words, E)
    similarity = np.dot(wd1_emb, wd2_emb)
