...
class KNN:
    ...
    def get_k_neighbors(self, point):
        ...
        """
        distances = []
        for idx, (label, feature) in enumerate(self.train_data):
            dist = self.distance_function(point, feature)
            distances.append((dist, idx, label))  # Store distance, index, and label for tie-breaking
        distances.sort(key=lambda x: (x[0], x[1]))  # Sort by distance, then by index
        num_neighbors = min(self.k, len(distances))  # Handle case where k is larger than available points
        k_neighbors = [self.labels[distances[i][1]] for i in range(num_neighbors)]
        return k_neighbors
    ...
    def predict(self, features):
        ...
        for point in features:
            k_neighbors = self.get_k_neighbors(point)
