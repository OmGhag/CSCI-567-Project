...
class QLearningAgent:
    ...
    def learn(self) -> dict:
        ...
        # Convert to int for indexing (safety)
        states = batch.states.astype(int)
        actions = batch.actions.astype(int)
        next_states = batch.next_states.astype(int)
        ...
        next_q_values = np.max(self.q_table[next_states], axis=1)
